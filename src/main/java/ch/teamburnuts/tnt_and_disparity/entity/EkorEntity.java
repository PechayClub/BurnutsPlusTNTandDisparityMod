...
public class EkorEntity extends Monster implements RangedAttackMob {
	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(TntAndDisparityModEntities.EKOR, 10, 1, 1));
	}

	public EkorEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(TntAndDisparityModEntities.EKOR, world);
	}

	public EkorEntity(EntityType<EkorEntity> type, Level world) {
		super(type, world);
		xpReward = 10;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(TntAndDisparityModItems.EKOR_PUPPET));
	}
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		EkorShooterEntity.shoot(this, target);
	}

	public static void init() {
		SpawnPlacements.register(TntAndDisparityModEntities.EKOR, SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
